...
- **Intégration DB** — la RLS (Row-Level Security) PostgreSQL est un contrôle de sécurité, pas un détail de schéma : elle doit être testée en tant que tel, avec un test qui échoue si une requête exécutée hors du contexte tenant attendu renvoie une ligne (cf. le piège déjà documenté ailleurs dans les mémoires du projet : un superutilisateur en dev masque ce que la RLS interdirait réellement — les tests d'intégration DB doivent s'exécuter avec le rôle applicatif réel, jamais un rôle privilégié).
...
Le protocole de preuve (design V1 §4.5) est calculé indépendamment côté TypeScript (signature client) et côté Go (vérification serveur). La garantie que les deux implémentations produisent des octets identiques n'est pas assumée, elle est prouvée par une fixture committée (`testvectors/proof-event-fixture.json`, générée par `packages/kofra-protocol/scripts/generate-proof-fixture.ts` et vérifiée par `control-plane/internal/proof/hashchain_test.go`) : un changement qui casse cette parité échoue en CI plutôt que de dériver silencieusement en production.
